        		...
        		<div class="col-sm-12">
                        <div class="white-box">
                            ...
                            <div class="panel-heading m-b-15">
            						...
            						<a class="btn btn-info btn-rounded" href="{{ route('tags.create') }}"> Add Tags</a>
            				</div>
                                ...
                                <table id="myTable" class="table table-striped">
                                    ...
                                    <tbody>
                                        ...
                                        <tr >
                                            <td >{{ $i }}</td>
                                            <td>{{ $data->name }}</td>
                                            <td><div class="color-box" style="background-color:<?php echo $data['color']; ?>;"></div></td>
                                            <td>@php($status_chk='')
											@if ($data->status == 1)
												@php($status_chk='checked="checked"')
											@endif
												<div class="onoffswitch" >
												<input type="checkbox" name="onoffswitch" class="onoffswitch-checkbox changestatus" id="switch_sick{{ $i }}" data-table="tags" data-field="status" data-id-name="id" data-id="{{ $data->id }}" {{ $status_chk }}  value="1">
												<label class="onoffswitch-label" for="switch_sick{{ $i }}">
													<span class="onoffswitch-inner"></span>
													<span class="onoffswitch-switch"></span>
												</label>
												</div></td>
                                            <td><a href="{{ route('tags.edit',$data['id']) }}"><i class="fa fa-edit"></i></a> &nbsp;&nbsp;<?php if($data['id'] > 12){ ?> <a href="javascript:void(0)" data-id="{{ $data->id }}" class="data_delete"><i class="fa fa-trash"></i></a> <?php }?> </td>
                                        </tr>
                                        @php($i++)
										@endforeach
                                    </tbody>
